...
Switch::Switch(bool value, QWidget *parent) : QCheckBox(parent)
{
...
void Switch::paintEvent(QPaintEvent *paintEvent)
{
    ...
    switchOutline.addRoundedRect(
            QRect { QPoint { drawRect.x(),
                             drawRect.y() + static_cast<int>(.15f * drawRect.height()) },
                    QSize { static_cast<int>(std::roundf(1.5f * drawRect.height())),
                            static_cast<int>(std::roundf(.5f * drawRect.height())) } },
            static_cast<int>(.75 * circleRadius), static_cast<int>(.75 * circleRadius));
    QPainterPath switchToggler;

    if (!this->isChecked()) {
        switchToggler.addEllipse(
                QPoint { drawRect.x() + circleRadius, drawRect.y() + circleRadius }, circleRadius,
                circleRadius);
    } else {
        switchToggler.addEllipse(
                QPoint { drawRect.x() + circleRadius
                                 + static_cast<int>(
                                         std::roundf(1.5 * drawRect.height() - (circleRadius * 2))),
